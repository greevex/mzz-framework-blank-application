...
{form action={url} method="post" jip=true}
    <table border="0" width="50%" cellpadding="4" cellspacing="1" class="systemTable">
        ...
        {/if}
        <tr>
            <td colspan="2"><a href="{url route=withAnyParam module=user action=addToGroup name=$group->getId()}" class="mzz-jip-link" title="{_ add_user_to_group}">{icon sprite="sprite:sys/user-add"} {_ add_user_to_group}</a></td>
        </tr>
    </table>
</form>