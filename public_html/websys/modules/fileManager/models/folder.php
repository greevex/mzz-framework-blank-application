<?php
/**
 * $URL: svn://svn.mzz.ru/mzz/trunk/system/modules/fileManager/models/folder.php $
 *
 ...
 * @version $Id: folder.php 3876 2009-10-22 02:40:05Z striker $
*/
...
/*
    ...
    public function upload($upload_name, $name = null)
    {
        ...
        while (true) {
            try {
                ...
                if ($file->getStorage()->moveUploadedFile($info['tmp_name'], $file->getRealname())) {
					break;
                }
    ...
    public function uploadViaHttp($address, $name = null)
    {
        $image_source = file_get_contents($address);
        if (!empty($image_source)) {
            $info = array('name' => basename($name), 'size' => strlen($image_source));
        } else {
            throw new mzzRuntimeException('Укажите файл для загрузки');
        }

        $image_path = systemConfig::$pathToTemp . '/' . $name;
        ...
        while (true) {
            try {
                ...
                $file->setStorage($storage);
				$fileMapper->save($file);
                if ($file->getStorage()->saveImageSource($image_source, $file->getRealname())) {
					break;
                }

                throw new mzzRuntimeException('Файл "' . $address . '" не был перемещён  в каталог "' . $storage->getPath() . '"');
            } catch (PDOException $e) {
            }
        }
