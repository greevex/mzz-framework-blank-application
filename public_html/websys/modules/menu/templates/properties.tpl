...
<table border="0" cellpadding="3" cellspacing="1" width="100%" class="grayBorder">
    ...
    <tr>
        <td style="width: 20%">{form->caption name="description" value="Подсказка:"}</td>
        <td style="width: 80%;">{form->text name="description" size=40 value=$item->getDescription()} {$validator->getFieldError('description')}</td>
    </tr>
    <tr>
        <td style="width: 20%">{form->caption name="groups" value="Только для групп:"}</td>
        <td style="width: 80%;">{form->select options=$groups keyMethod="getId" valueMethod="getName" name="groups[]" emptyFirst="Для всех" size=10 multiple=1 value=$item->getGroups()} {$validator->getFieldError('groups')}</td>
    </tr>
    <tr>
        <td style="width: 20%">{form->caption name="is_jip" value="С JIP:"}</td>
        <td style="width: 80%;">{form->checkbox value=$item->isJip() name="is_jip"} {$validator->getFieldError('is_jip')}</td>
    </tr>
