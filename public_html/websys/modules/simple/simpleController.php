<?php
/**
 * $URL: svn://svn.mzz.ru/mzz/trunk/system/modules/simple/simpleController.php $
 *
 ...
 * @version $Id: simpleController.php 4359 2010-11-10 14:01:39Z iLobster $
 */

fileLoader::load('modules/simple/flash');
...
abstract class simpleController
{
    ...
    public function __construct(simpleAction $action = null)
    {
        ...
        $jqOptions = $this->request->getArray('jqOptions', SC_GET);
        if($jqOptions && !empty($jqOptions)) {
            $respond = array('status' => 'false');
            if(isset($jqOptions['act'])) {
                switch($jqOptions['act']) {
                    case "get":
                        $respond = $this->options->get();
                        if(!$respond) {
                            $respond = array();
                        }
                        $respond['status'] = 'true';
                    ...
                    case "set":
                        ...
                        if(isset($jqOptions['status'])) {
                            $respond['status'] = 'true warning';
                            $respond['error'] = 'Warning! `status` key can not be rewrited!';
                        } else {
                            $respond['status'] = 'true';
                        }
                        $jqOptions = array_merge($options, $jqOptions);
                        $this->options->set($jqOptions);
                        $this->saveOptions;
                        break;
                    case "clear":
    ...
    private $DbSimple = null;

    private $simpleOptionsMapperInstance = null;

    private $cacheInstance = null;

    private $optionsEntity = null;

    public function __get($name)
    {
        switch($name) {
            case "db":
                if(is_null($this->DbSimple)) {
                    $this->DbSimple = DbSimpleInstance();
                }
                return $this->DbSimple;
                break;
            case "simpleOptionsMapper":
                if(is_null($this->simpleOptionsMapperInstance)) {
                    $this->simpleOptionsMapperInstance = $this->toolkit->getMapper('user', 'options');
                }
                return $this->simpleOptionsMapperInstance;
                break;
            case "options":
                $cache_key = "{$this->request->getModule()}_{$this->request->getAction()}_{$this->toolkit->getUser()->getId()}_options";
                if(is_null($this->optionsEntity)) {
                    if(!$this->cache->get($cache_key, $this->optionsEntity)) {
                        $this->optionsEntity = $this->simpleOptionsMapper->searchByNameInCurrentContext($cache_key);
                        $this->cache->set($cache_key, $this->optionsEntity);
                    }
                }
                return $this->optionsEntity;
                break;
            case "saveOptions":
                $cache_key = "{$this->request->getModule()}_{$this->request->getAction()}_{$this->toolkit->getUser()->getId()}_options";
                $this->simpleOptionsMapper->save($this->optionsEntity);
                $this->cache->set($cache_key, $this->optionsEntity);
                break;
            case "cache":
                if(is_null($this->cacheInstance)) {
                    $this->cacheInstance = cache::factory();
                }
