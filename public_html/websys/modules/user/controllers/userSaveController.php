...
class userSaveController extends simpleController
{
    protected function getView()
    {
        ...
        if (!$isEdit) {
            ...
            $validator->rule('email', 'user[email]', 'Неправильный email');
        }
        ...
        if ($validator->validate()) {
            ...
            $editedUser->setName($info['name']);
            $editedUser->setEmail($info['email']);

            if (isset($info['confirmed']) && $info['confirmed']) {
                $editedUser->setConfirmed('');
            } else {
                $editedUser->setConfirmed(md5(time()));
            }
